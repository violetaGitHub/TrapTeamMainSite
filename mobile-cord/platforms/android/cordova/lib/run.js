...
module.exports.run = function (runOptions) {
    ...
    return Q().then(() => {
        if (!install_target) {
            ...
            return device.list().then((device_list) => {
        ...
        if (install_target === '--device') {
            ...
            return emulator.list_started().then((started) => {
        ...
        return device.list().then((devices) => {
            if (devices.indexOf(install_target) > -1) {
                return device.resolveTarget(install_target);
            }
            return emulator.list_started().then((started_emulators) => {
                if (started_emulators.indexOf(install_target) > -1) {
                    return emulator.resolveTarget(install_target);
                }
                return emulator.list_images().then((avds) => {
                    // if target emulator isn't started, then start it.
                    for (var avd in avds) {
                        if (avds[avd].name === install_target) {
                            return emulator.start(install_target).then((emulatorId) => {
                                return emulator.resolveTarget(emulatorId);
                            });
                        }
                    }
        ...
        return new Promise((resolve) => {
            ...
            if (resolvedTarget && resolvedTarget.isEmulator) {
                return emulator.wait_for_boot(resolvedTarget.target).then(() => {
