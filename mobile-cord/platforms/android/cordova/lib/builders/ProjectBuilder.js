...
class ProjectBuilder {
    ...
    */
    ...
    prepBuildFiles () {
        ...
        var settingsGradlePaths = subProjects.map((p) => {
        ...
        propertiesObj.systemLibs.forEach((p) => {
            var mavenRef;
        ...
        propertiesObj.gradleIncludes.forEach((includePath) => {
        ...
        return spawn(wrapper, args, { stdio: 'pipe' })
            .progress((stdio) => {
                ...
                if (error.toString().indexOf('failed to find target with hash string') >= 0) {
                    return check_reqs.check_android_target(error).then(() => {
...
function findOutputApksHelper (dir, build_type, arch) {
    ...
    var ret = shell.ls('-R', dir).map((file) => {
    ...
    var archSpecific = Boolean(/-x86|-arm/.exec(path.basename(ret[0])));
    // And show only arch-specific ones (or non-arch-specific)
    ret = ret.filter((p) => {
        return Boolean(/-x86|-arm/.exec(path.basename(p))) === archSpecific;
    });
